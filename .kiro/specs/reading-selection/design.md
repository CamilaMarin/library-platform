...
- A book can only appear as a draw candidate if EVERY selected participant has authorized access:
  - Physical book: participant owns a Copy with `type == physical` and `status == available`.
