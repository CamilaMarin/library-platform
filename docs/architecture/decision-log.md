...
| 2025-07-27 | Application-level encryption at rest deferred to post-MVP | No real user data before legal review gate (Task 10); infrastructure-level encryption sufficient for pre-launch; key management complexity disproportionate for MVP | Implement now, infrastructure-only permanent, partial (files only) | Medium — must revisit before Dec 2026 enforcement; retrofitting requires migration if legal review demands field-level encryption | ADR-0018 | privacy | M8 |
